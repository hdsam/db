...
public class BPlusTree<K extends Comparable<? super K>, V> {
	...
	public BPlusTree(int branchingFactor) {
		if (branchingFactor <= 2) {
			throw new IllegalArgumentException("不合法的分支数量：" + branchingFactor);
		}
	...
	private class InternalNode extends Node {
		...
		@Override
		void merge(BPlusTree<K, V>.Node sibling) {
			@SuppressWarnings("unchecked")
			InternalNode node = (BPlusTree<K, V>.InternalNode) sibling;
		...
		@Override
		BPlusTree<K, V>.Node split() {
			int from = keyNumber() / 2 + 1, to = keyNumber();
			InternalNode sibling = new InternalNode();
			sibling.keys.addAll(keys.subList(from, to + 1));
		...
		private BPlusTree<K, V>.Node getChild(K key) {
		...
		void insertChild(K key, BPlusTree<K, V>.Node child) {
			int loc = Collections.binarySearch(keys, key);
			int childIndex = loc >= 0 ? loc + 1 : -loc - 1;
			if (loc >= 0) {
		...
		BPlusTree<K, V>.Node getChildLeftSibling(K key) {
			int loc = Collections.binarySearch(keys, key);
			int childIndex = loc >= 0 ? loc + 1 : -loc - 1;
			if (childIndex > 0) {
		...
		BPlusTree<K, V>.Node getChildRightSibling(K key) {
			int loc = Collections.binarySearch(keys, key);
			int childIndex = loc >= 0 ? loc + 1 : -loc - 1;
